...
However, in scientific applications, we not only target one application on the systems, and one specific hyperparameter configuration showcaseing the scalability of the system, but we are interested in finding out which application and which hyperparameters for that application are best to achieve the best accuracy. Hence, it is important to improve the accuracy while identifying guidelines for resource consumption to achieve the best accuracy promoting the FAIR principle \cite{www-fair}. 
...
such different systems. Instead, 
% we list some of the systems and organize them in various categories within this section. 
we refer to \cite{las-22-arxiv-workflow-cc} where a more detailed analysis is provided. One of the systems that has been developed recently as part of MLCommons recently is MLCube \cite{www-mlcube}. While some aspects are similar, our system has a different focus when applied to the applications.
...
To fulfill our requirements, we have developed a sophiticated but easy to use framework for controlling several aspects of our workflow needs: (a) {\em System Integration}: the placement of the workflow onto a number of different compute resources including HPC, cloud, and local computing while also providing adaptations to various batch queues, (b) {\em Compute Coordination}: the coordination of task-based parallelelism to execute the workflow on the various resources, and (c) {\em Experiment Executor\footnote{The repository and name is at this time called cloudmesh-sbatch, but will be renamed to cloudmesh-ee to reflect the better name of Experiemnet Executor}}: the coordination of hyperparameter sweeps used in the application that we call experiment coordinator. The architecture of the
...
and expanded on at each layer. We distinguish multiple users including developers, 
commandline and service users, that define their workflows to run on
computing resources. 

For the end user, we provide a command line and a
